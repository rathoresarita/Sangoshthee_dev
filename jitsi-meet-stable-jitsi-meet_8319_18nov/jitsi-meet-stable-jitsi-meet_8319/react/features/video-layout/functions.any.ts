...
import { getParticipantCount, getPinnedParticipant,getRemoteModeratorsCount } from '../base/participants/functions';
...
export function shouldDisplayTileViewForModerator(state: IReduxState){
    const { tileViewEnabled } = state['features/video-layout'] ?? {};
    console.log('tileviewE',tileViewEnabled)
    ...
    const tileViewEnabledFeatureFlag = getFeatureFlag(state, TILE_VIEW_ENABLED, false);
    // Set disableTileView to true
//state['features/base/config'].disableTileView = true;

// Rest of your code
const { disableTileView } = state['features/base/config'];

    if (disableTileView || !tileViewEnabledFeatureFlag) {
        console.log('tileviewD',tileViewEnabled)

        return false;
    }


   // const ModeratorCount = getRemoteModeratorsCount(state);
   const participantCount = getParticipantCount(state);
...
console.log("shouldDisplayNormalMode",shouldDisplayNormalMode)
    return !shouldDisplayNormalMode;
