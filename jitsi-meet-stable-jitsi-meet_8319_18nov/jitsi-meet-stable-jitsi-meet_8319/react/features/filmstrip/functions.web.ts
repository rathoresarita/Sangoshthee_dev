...
//         const numberOfParticipants= getRemoteModeratorsCount(state)
//         return numberOfParticipants- (iAmRecorder ? 1 : 0)
...
// export function getNumberOfPartipantsForTileView(state: IReduxState): number {
//     const { iAmRecorder } = state['features/base/config'];
//     const disableSelfView = getHideSelfView(state);
//     const isModerator = isLocalParticipantModerator(state);
...
//         const { localScreenShare } = state['features/base/participants'];
//         const localParticipantsCount = localScreenShare ? 2 : 1;
//         const numberOfParticipants = getRemoteModeratorsCount(state) - (iAmRecorder ? 1 : 0) - (disableSelfView ? localParticipantsCount : 0);
//         return numberOfParticipants >= 0 ? numberOfParticipants : 0; // Ensure it's not negative
//     }
...
export function getNumberOfPartipantsForTileView(state: IReduxState) {
    ...
    const { localScreenShare } = state['features/base/participants'];
    const localParticipantsCount = localScreenShare ? 2 : 1;
    const ParticipantCount= getRemotePCount(state)
    const numberOfParticipants = getParticipantCountWithFake(state)
...
/**
 * Returns the number of moderators for tile view.
 *
 * @param {Object} stateful - The redux store state.
 * @returns {number} The number of moderators in tile view.
 */
export function getModeratorCountForTileView(stateful: IReduxState){
    const state = toState(stateful)['features/base/participants'];
    const remoteParticipants = state.remote;
    
    const numberOfParticipants = getNumberOfPartipantsForTileView(stateful);
  
  
    const moderatorCount = Array.from(remoteParticipants.values()).filter(participant => participant.role === 'moderator').length;
    if (numberOfParticipants % moderatorCount === 0) {
        return moderatorCount
    } 
    
    else {
        // const adjustedParticipants = numberOfParticipants + 1;
        // console.log('adjustedParticipants',adjustedParticipants)
        // console.log('ModeratorC',moderatorCount )
        // Adjust the moderator count or handle the case where the condition is false
        const adjustedModeratorCount = Math.ceil(numberOfParticipants / moderatorCount);
        return adjustedModeratorCount;
    }
...
export function calculateNonResponsiveTileViewDimensions(state: IReduxState) {
    ...
    if (typeof size === 'undefined') {
        ...
        const width = clientWidth / c; // Adjusting width based on the number of columns
        const height = width / aspectRatio; // Calculating height based on aspect ratio
...
export function  calculateNonResponsiveTileViewDimensionsForModerator(state: IReduxState) {
    const { clientHeight, clientWidth } = state['features/base/responsive-ui'];
    const { disableTileEnlargement } = state['features/base/config'];
    const { columns: c, minVisibleRows, rows: r } = getNotResponsiveTileViewGridDimensions(state);
    const size = calculateThumbnailSizeForTileView({
...
console.log('no of partcipant',numberOfParticipants)
console.log('desiredNumberOfVisibleTiles',numberOfParticipants)
...
console.log('coloumns',c)

console.log('clientWidth',clientWidth,)
console.log('clientHeight',clientHeight,)
console.log(' disableTileEnlargement,', disableTileEnlargement)
console.log('noHorizontalContainerMargin',noHorizontalContainerMargin)
console.log('minTileHeight',minTileHeight)
console.log('size',size)
        ...
        if (size) {
            ...
            console.log(' numberOfVisibleParticipants', numberOfVisibleParticipants)
            let area = Math.round(
...
//for Moderator
export function calculateResponsiveTileViewDimensionsForModerator({
    clientWidth,
    clientHeight,
   disableTileEnlargement = false,
   noHorizontalContainerMargin = false,
    ...
    for (let c = 1; c <= Math.min( maxColumns,numberOfModerators, desiredNumberOfVisibleTiles); c++) {
      
        const r = Math.ceil(numberOfModerators / c);

          console.log('Iteration:', c);
            console.log('maxColumns:', maxColumns);
            console.log('numberOfModerators:', numberOfModerators);
            console.log('desiredNumberOfVisibleTiles:', desiredNumberOfVisibleTiles);
            // ...
        ...
        // we want to display as much as possible tumbnails up to desiredNumberOfVisibleTiles
        const visibleRows
            = numberOfModerators <= desiredNumberOfVisibleTiles ? r : Math.floor(desiredNumberOfVisibleTiles / c);
console.log('visibleRows',visibleRows)
        const size = calculateThumbnailSizeForTileView({
            columns: c,
            minVisibleRows: visibleRows,
...
console.log('size',size)
        ...
        if (size) {
            const { height: currentHeight, width: currentWidth, minHeightEnforced, maxVisibleRows } = size;
            const numberOfVisibleParticipants = Math.min(c * maxVisibleRows, numberOfModerators);
            console.log(' numberOfVisibleParticipants', numberOfVisibleParticipants)
            let area = Math.round(
                (currentHeight + TILE_VERTICAL_MARGIN)
                * (currentWidth + TILE_HORIZONTAL_MARGIN)
            ...
            if (!minHeightEnforced) {
                if (area > dimensions.maxArea) {
                    ...
                    && ((oldNumberOfVisibleParticipants > desiredNumberOfVisibleTiles
                            && oldNumberOfVisibleParticipants >= numberOfModerators)
                        || (oldNumberOfVisibleParticipants < numberOfModerators
                            && numberOfVisibleParticipants <= desiredNumberOfVisibleTiles))
                ) { // If the area of the new candidates and the old ones are equal we prefer the one that will have
                    // closer number of visible participants to desiredNumberOfVisibleTiles config.
                ...
                area = currentHeight * currentWidth * Math.min(c, numberOfModerators);
