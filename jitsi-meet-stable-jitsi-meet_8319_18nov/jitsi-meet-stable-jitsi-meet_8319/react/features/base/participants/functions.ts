...
export function getOneModerator(stateful: IStateful): IParticipant | undefined {
    const participants = toState(stateful)['features/base/participants'].remote

    // Find the participant with the role set to 'moderator'
    for (const participant of participants.values()) {
        if (participant.role === 'moderator') {
            console.log('modparticipant',participant)
    ...
    return undefined; // Return undefined if no moderator is found
}
...
export function getDominantSpeakerOrLastModeratorId(stateful: IStateful): string | null {
    const moderators = getModerator(stateful);

    // Find the moderator with dominantspeaker set to true
    const dominantSpeakerModerator = moderators.find((moderator) => moderator.dominantSpeaker);

    if (dominantSpeakerModerator) {
        const dominantSpeakerModeratorId = dominantSpeakerModerator.id;
        console.log('Dominant speaker moderator id', dominantSpeakerModeratorId);
        return dominantSpeakerModeratorId;
    } else {
        // If no moderator with dominantspeaker set to true, return the ID of the last moderator
        const lastModerator = moderators.length > 0 ? moderators[moderators.length - 1].id : null;
        console.log('Last moderator id', lastModerator);
        return lastModerator;
    }
}


export function getModeratorId(stateful: IStateful): string | undefined {
    const participants = toState(stateful)['features/base/participants'].remote;

    // Find the participant with the role set to 'moderator'
    for (const participant of Object.values(participants)) {
        if (participant.role === 'moderator') {
            console.log('Moderator participant ID:', participant.id);

            return participant.id ; // Return the ID of the moderator
        }
    }
...
// Modify getModeratorIds to return an array of moderator IDs
export function getModeratorIds(stateful: IStateful): string[] {
    const participants = toState(stateful)['features/base/participants'].remote;
    const moderatorIds: string[] = [];

    // Find all participants with the role set to 'moderator'
    for (const participant of participants.values()) {
        if (participant.role === 'moderator') {
            moderatorIds.push(participant.id);
        }
    }
console.log('get moderator idssss',moderatorIds)
    return moderatorIds; // Return an array of moderator IDs
}

export function getLastModeratorId(stateful: IStateful): string | null {
    const moderatorIds = getModeratorIds(stateful);

    // Check if there are any moderator IDs
    if (moderatorIds.length > 0) {
        const lastModeratorId = moderatorIds[moderatorIds.length - 1];
        console.log('last moderator id', lastModeratorId);
        return lastModeratorId;
    } else {
        console.log('No moderator IDs found');
        return null;
    }
}
...
// export function getModeratorById(stateful: IStateful, moderator  Id: string): IParticipant | undefined {
//     const state = toState(stateful)['features/base/participants'];
//     const remoteParticipants = state.remote;
//     console.log('GetModeratorby idddddd',remoteParticipants.get(moderatorId))
//     return remoteParticipants.get(moderatorId);
...
// export function getModeratorById(state: IStateful) {
//     const participantsMap = toState(state)['features/base/participants'].remote;
...
//     // Filter participants by 'moderator' role
//     const moderatorParticipants = participantsArray.filter(participant => participant.role === 'moderator');
// console.log('moderatorParticipants...........',moderatorParticipants)
    
    
//     return moderatorParticipants; // Return the moderator object or undefined if not found
// }


// export function getModeratorById(state: IStateful): IParticipant[] {
//     const participantsMap = toState(state)['features/base/participants'].remote;

//     // Convert the Map to an array
//     const participantsArray = Array.from(participantsMap.values());

//     // Filter participants with the 'moderator' role
//     const moderators = participantsArray.filter(participant => participant.role === 'moderator');

//     console.log('moderators:', moderators);
//     return moderators; // Return an array of moderator objects
// }
...
export function getRemotePCount(stateful: IStateful) {
    const state = toState(stateful);
    const remoteParticipants = state['features/base/participants'].remote;
    ...
    // Assuming that the role information is stored in the participant data
const moderatorParticipants = new Map<string, IParticipant>(
    [...remoteParticipants].filter(([id, participant]) => participant.role === 'participant')
);
...
export function isParticipantLocal(participant?: IParticipant){
    return participant?.role === PARTICIPANT_ROLE.PARTICIPANT;
}
/**
...
export function getDominantSpeakerModerator(stateful: IStateful) {
    
    const state = toState(stateful)['features/base/participants'];
    const { remote } = state;

    const remoteArray = Array.from(remote.values());
    const moderators = remoteArray.filter((participant) => participant.role === 'moderator' );

    const moderatorIds = moderators.map((participant) => participant.id);
    console.log('Moderator IDs:', moderatorIds);

    const dominantSpeakerModerators = moderators.filter((moderator) => moderator.dominantSpeaker);

    if (dominantSpeakerModerators.length > 0) {
        const dominantSpeakerModeratorId = dominantSpeakerModerators[0].id;
        console.log('Dominant speaker moderator id', dominantSpeakerModeratorId);
         return getParticipantById(stateful, dominantSpeakerModeratorId);;
    }
    if (dominantSpeakerModerators) {
        return undefined;
    }
...
export const isNonPanelistMemberModerator = createSelector(
  getLocalParticipant,
  localParticipant => {
    if (!localParticipant) {
      return false;
...
export function isParticipantNotModerator( stateful: IStateful) {
    const state = toState(stateful)['features/base/participants'];
    ...
    return  isParticipantLocal(local);
}
